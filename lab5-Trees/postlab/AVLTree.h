...
private:
  ...
  int height(AVLNode*& node, const string& x, int count);
  ...
  bool find(AVLNode*& node, const string& x);

  string pathTo(AVLNode*& node, const string& x, string result);

  void insert(AVLNode*& node, const string& x);

  int numNodes(AVLNode*& node);
