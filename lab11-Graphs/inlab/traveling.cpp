...
/*********
 *! \brief This will solve the traveler's problem
 *\date  4/26/2022
 *\author  Helen Yao
 *********/
...
int main(int argc, char** argv) {
    ...
    vector<string> actualDests;
    // TODO: YOUR CODE HERE
    for(int i = 0; i<dests.size(); i++){
      actualDests.push_back(dests[i]);
    }
    
    
    float minDis = computeDistance(me, start,dests);
    //cout<<"Beginning distance: " << minDis<<endl;
    //printRoute(start,dests);
    //cout<<endl;
    sort(dests.begin(),dests.end());      
    while(next_permutation(dests.begin(), dests.end())){
      if(computeDistance(me, start,dests) < minDis){
	  minDis = computeDistance(me, start, dests);
	  actualDests= dests;
      }
...
float computeDistance(MiddleEarth& me, const string& start, vector<string> dests) {
    // TODO: YOUR CODE HERE
  float totalDis = me.getDistance(start, dests[0]);
  for(int i = 0; i<dests.size()-1; i++){
    totalDis += me.getDistance(dests[i], dests[i+1]);
  }
  totalDis+=me.getDistance(dests[dests.size()-1],start);
    return totalDis;
}
